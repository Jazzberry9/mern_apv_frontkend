import { useState, useEffect } from "react";
import Alert from "../components/Alert";
import usePacientes from "../hooks/usePacientes"; 

const Formulario = () => {

  const [ nombre, setNombre ] = useState('');
  const [ propietario, setPropietario ] = useState('');
  const [ email, setEmail ] = useState('');
  const [ fecha, setFecha ] = useState('');
  const [ sintomas, setSintomas ] = useState('');
  const [ id, setId ] = useState(null);

  const [ alerta, setAlerta ] = useState({});
  const { guardarPacientes, paciente } = usePacientes();

  useEffect( () => {
    if(paciente?.nombre){
      setNombre(paciente.nombre);
      setPropietario(paciente.propietario);
      setFecha(paciente.fecha);
      setEmail(paciente.email);
      setSintomas(paciente.sintomas);
      setId(paciente._id);
    }
  }, [paciente])

  const handleSubmit = ev => {
    ev.preventDefault();

    // validar el formulario
    if([nombre,propietario,email,fecha,sintomas].includes('')){
      setAlerta({
        msg: "Todos los campos son obligatorios",
        error: true
      })
      return;
    }
    // actualiza alerta
    setAlerta({msg: "Guardado correctamente", error: false});
    // guarda en funcion del context
    guardarPacientes({ nombre,propietario,email,fecha,sintomas, id });
    // resetea el form
    setNombre('');
    setPropietario('');
    setEmail('');
    setFecha('');
    setSintomas('');
    setId('');
  }

  const { msg } = alerta;
  return (
    <>
      <h2 className="font-black text-3xl text-center">Administrador</h2>
      <p className="text-xl mt-5 text-center mb-10">
        Agrega a tus Pacientes y <span className="text-teal-600 font-bold">Administralos</span>
      </p>

      <form 
        className="bg-white py-10 px-5 mb-10 lg:mb-0 shadow-md rounded-md"
        onSubmit={handleSubmit}
      >

        {/* Input mascota */}
        <div className="mb-5">
          <label 
          htmlFor="nombre"
          className="text-gray-700 uppercase font-bold"
          >Mascota</label>
          
          <input 
            type="text" 
            id="nombre"
            placeholder="Nombre de la mascota"
            className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md"
            value={nombre}
            onChange={ev => setNombre(ev.target.value)}
          
          />
        </div>

        {/* Input propietario */}
        <div className="mb-5">
          <label 
          htmlFor="propietario"
          className="text-gray-700 uppercase font-bold"
          >Propietario</label>
          
          <input 
            type="text" 
            id="propietario"
            placeholder="Nombre del propietario"
            className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md"
            value={propietario}
            onChange={ev => setPropietario(ev.target.value)}
          />
        </div>

        {/* Input email */}
        <div className="mb-5">
          <label 
          htmlFor="email"
          className="text-gray-700 uppercase font-bold"
          >Email</label>
          
          <input 
            type="text" 
            id="email"
            placeholder="Email del propietario"
            className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md"
            value={email}
            onChange={ev => setEmail(ev.target.value)}
          />
        </div>

        {/* Input fecha */}
        <div className="mb-5">
          <label 
          htmlFor="fecha"
          className="text-gray-700 uppercase font-bold"
          >Fecha Alta</label>
          
          <input 
            type="date" 
            id="fecha"
            className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md"
            value={fecha}
            onChange={ev => setFecha(ev.target.value)}
          />
        </div>

        {/* Input sintomas */}
        <div className="mb-5">
          <label 
          htmlFor="sintomas"
          className="text-gray-700 uppercase font-bold"
          >Sintomas</label>
          
          <textarea 
            name="" 
            cols="30" 
            rows="7"
            id="sintomas"
            placeholder="Describa los sintomas del paciente"
            className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md"
            value={sintomas}
            onChange={ev => setSintomas(ev.target.value)}
          >

          </textarea>
        </div>

        <input 
          type="submit"
          value={ id ? "Guardar cambios" : 'Agregar paciente'}
          className="bg-teal-500 text-white uppercase font-bold w-full p-3 hover:bg-teal-600 cursor-pointer transition-colors rounded-md mb-5"
        
        />

        {msg && <Alert 
          alerta={alerta}
        />}

      </form>
    
    </>
  )
}
export default Formulario